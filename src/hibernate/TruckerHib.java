...
public class TruckerHib {
    ...
    public Trucker getTruckerByLoginData(String login, String psw) {
        entityManager = emf.createEntityManager();
        Query query = null;
        ...
        queryManager.select(root).where(cb.and(cb.like(root.get("login"), login), cb.like(root.get("password"), psw)));
